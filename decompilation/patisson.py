...
camera_speed = 10
camera_rotation_speed = 1000
sound_volume = 1
music_volume = 1
master_volume = 1
harvest_active = False
harvest_clicks = 0
harvest_clicks_needed = 10
game_ended = False
end_text = None
...
sounds = {
    'click': Audio('click.wav', autoplay = False, volume = 1),
    'cash': Audio('Cash.wav', autoplay = False, volume = 1),
    'nope': Audio('nope.wav', autoplay = False, volume = 1),
    'death': Audio('aaaa.wav', autoplay = False),
    'hurt': Audio('hurt.wav', autoplay = False),
    'well': Audio('Колодец.wav', autoplay = False, volume = 1),
    'fishing': Audio('Удочка.wav', autoplay = False, volume = 1),
    'cast_rod': Audio('Закидывание.wav', autoplay = False, volume = 1) }
menu_tracks = [
    'Married Life.mp3',
    'Gymnopedie No.mp3',
    'Summer Harvest Theme.mp3']
game_tracks = [
    'Wynton Marsalis.mp3',
    'Classic Easter.mp3',
    'Bee Swarm.mp3',
    'Spring.mp3',
    'Rondo Alla Turca Mozart.mp3']
...
def start_menu_music():
    ...
    try:
        ...
        invoke(next_menu_track, delay = 180)
        return None
...
def start_game_music():
    ...
    try:
        ...
        invoke(next_game_track, delay = 180)
        return None
...
def start_tutorial():
    global tutorial_step, tutorial_active, game_paused
    tutorial_step = 1
...
play_button = Button(parent = main_menu_panel, model = 'quad', texture = 'Играть.png', scale = (0.28, 0.17), position = (-0.31, 0.19), color = color.white, z = -0.02, enabled = True, on_click = (lambda : start_game()))
settings_button = Button(parent = main_menu_panel, model = 'quad', texture = 'Настройки.png', scale = (0.28, 0.17), position = (-0.31, 0.01), color = color.white, z = -0.02, enabled = True, on_click = (lambda : open_settings()))
achievements_button = Button(parent = main_menu_panel, model = 'quad', texture = 'Достижения.png', scale = (0.28, 0.17), position = (-0.31, -0.17), color = color.white, z = -0.02, enabled = True, on_click = (lambda: None))
exit_button_main = Button(parent = main_menu_panel, model = 'quad', texture = 'Выход.png', scale = (0.28, 0.17), position = (-0.31, -0.35), color = color.white, z = -0.02, enabled = True, on_click = application.quit)
...
sound_button = Button(parent = settings_panel, model = 'quad', texture = 'Звук.png', scale = (0.3, 0.15), position = (0, 0.15), color = color.white, z = -0.02, enabled = False, on_click = (lambda : open_sound_settings()))
controls_info_button = Button(parent = settings_panel, model = 'quad', texture = 'Управление.png', scale = (0.3, 0.15), position = (0, -0.05), color = color.white, z = -0.02, enabled = False, on_click = (lambda : open_controls_info()))
settings_back = Button(parent = settings_panel, text = 'Назад', scale = (0.2, 0.1), position = (0, -0.25), color = color.red, text_color = color.white, enabled = False, on_click = (lambda : close_settings()))
...
def open_settings():
    global settings_from_pause, game_paused
    settings_from_pause = False
    toggle_elements([
        main_menu_bg,
        main_menu_panel,
        ...
        settings_button,
        achievements_button,
        exit_button_main], False)
    toggle_elements([
        settings_panel,
        settings_title,
        sound_button,
        controls_info_button,
        settings_back], True)
    game_paused = True
    mouse.locked = False
    mouse.visible = True
...
def open_pause_settings():
    ...
    toggle_elements([
        ...
        pause_achievements_button,
        exit_to_menu_button], False)
    toggle_elements([
        settings_panel,
        settings_title,
        sound_button,
...
def close_settings():
    global game_paused, game_paused
    toggle_elements([
        settings_panel,
        settings_title,
        sound_button,
...
continue_button = Button(parent = menu_panel, model = 'quad', texture = 'ПРигру.png', scale = (0.4, 0.1), position = (0, 0.3), color = color.white, z = -0.02, enabled = False, on_click = (lambda : toggle_menu()))
pause_settings_button = Button(parent = menu_panel, model = 'quad', texture = 'Настройки3.png', scale = (0.4, 0.1), position = (0, 0.1), color = color.white, z = -0.02, enabled = False, on_click = (lambda : open_pause_settings()))
pause_achievements_button = Button(parent = menu_panel, model = 'quad', texture = 'Достижения3.png', scale = (0.4, 0.1), position = (0, -0.1), color = color.white, z = -0.02, enabled = False, on_click = (lambda: None))
exit_to_menu_button = Button(parent = menu_panel, model = 'quad', texture = 'ВыходГМ.png', scale = (0.4, 0.1), position = (0, -0.3), color = color.white, z = -0.02, enabled = False, on_click = (lambda : show_confirm_exit()))
...
def exit_to_main_menu():
    global menu_visible, game_paused, end_text, end_menu_button, tutorial_active, main_menu_active
    menu_visible = False
    game_paused = False
    if end_text:
        destroy(end_text)
...
shop_items_list = [
        ...
        'type': 'fertilizer' },
    {
        ...
        'type': 'seedling' },
    {
        'image': 'ЗолотаяЛейка2.png',
        'price': 50,
        'type': 'golden_watering_can' },
    {
        'image': 'ЗачарованнаяУдочка2.png',
        'price': 100,
        'type': 'enchanted_fishing_rod' }]
shop_panel = Entity(parent = camera.ui, model = 'quad', scale = (1, 0.8), color = color.black66, enabled = False, z = -0.1)
...
dev_input = InputField(parent = dev_panel, scale = (0.6, 0.1), position = (0, 0), limit_content_to = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ: ', placeholder_text = '', color = color.gray, text_origin = (-0.5, 0), text_scale = 0.8, character_limit = 50, clip_content = True)
...
for i, icon in enumerate(inventory_icons):
    ...
    if i < 2:
        ...
        counter_text = Text(parent = slot, text = '0', position = (0.2, -0.2), scale = 8, color = color.white, alpha = 1, z = -0.02, enabled = False)
...
def toggle_ui(state = None):
    ...
    for element in ui_elements:
        if not element:
            continue
        element.enabled = ui_visible
    for slot in inventory_slots:
        if not slot:
            continue
        slot.enabled = ui_visible
    for counter in inventory_counters:
        if not counter:
            continue
        counter.enabled = ui_visible
    for shadow in inventory_shadows:
        if not shadow:
            continue
        shadow.enabled = ui_visible
...
def toggle_menu():  # [reconstructed]
    ...
    toggle_elements([
        ...
        pause_settings_button,
        pause_achievements_button,
        exit_to_menu_button], menu_visible)
...
def toggle_dev_panel():  # [reconstructed]
    global game_paused
    ...
    toggle_elements([
        ...
        dev_input,
        dev_apply_button,
        dev_placeholder], dev_panel.enabled)
...
def apply_cheat():  # [reconstructed]
    global player_coins, plant_growth, growth_speed_multiplier, immortal_plant, immortal_plant, free_camera_active, free_camera_active, camera_rotation_speed
...
def update_seedling_icon():
    if seedling_icon:
        seedling_icon.enabled = has_seedling
        return None


def update_watering_can_icon():  # [reconstructed]
    if watering_can_icon:
        watering_can_icon.texture = 'ЗолотаяЛейка.png' if has_golden_watering_can else 'Лейка.png'

def update_fishing_rod_icon():  # [reconstructed]
    if fishing_rod_icon:
...
def start_well_animation():
    global well_active, well_holding, well_sound_playing, well_total_rotations, well_rotation_time, prev_mouse_angle, game_paused
    if well_active or free_camera_active:
        return None
...
def finish_well_animation():
    global well_active, well_holding, well_sound_playing, game_paused, player_water
    well_active = False
    well_holding = False
    well_sound_playing = False
    game_paused = False
...
def start_fishing():  # [reconstructed]
...
def create_fishing_button():  # [reconstructed]
...
def fishing_button_clicked():
    global fishing_clicks, player_fish
    fishing_clicks += 1
...
def finish_fishing():
    global fishing_button, fishing_active, game_paused
    if fishing_button:
        destroy(fishing_button)
        fishing_button = None
    play_sound('fishing')
...
def start_game():
    global main_menu_active, game_paused, tutorial_active, tutorial_step
    reset_game()
    ...
    tutorial_active = True
    tutorial_step = 0
    invoke((lambda : toggle_elements([
tutorial_panel,
tutorial_text,
tutorial_yes_button,
tutorial_no_button], True)), delay = 1)
...
def start_harvest():  # [reconstructed]
...
def create_harvest_button():  # [reconstructed]
...
def finish_harvest():
    global harvest_button, harvest_active, game_paused, plant, end_text, end_menu_button, end_text, end_menu_button, game_ended
    if harvest_button:
        destroy(harvest_button)
        harvest_button = None
    harvest_active = False
