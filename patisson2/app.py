...
def configure(cfg: Config, offscreen: bool = False):
    ...
    loadPrcFileData("", "window-title Патиссон гейм 2.0")
    # The window and taskbar carry the game's own patisson rather than the
    # interpreter's logo. Generated by tools/make_icon.py out of the ripe
    # model, so it can never depict something the game does not contain.
    icon = Path(__file__).resolve().parent.parent / "assets" / "icon.ico"
    if icon.exists():
        loadPrcFileData("", f"icon-filename {Filename.fromOsSpecific(str(icon))}")
