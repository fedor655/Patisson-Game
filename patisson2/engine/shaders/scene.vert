...
void main() {
    ...
    if (u_wind.w > 0.0) {
        vec3 worldBase = (p3d_ModelMatrix * vec4(0.0, 0.0, 0.0, 1.0)).xyz;
        float height = max(modelPos.z - u_windPivot, 0.0);
        float phase = dot(worldBase.xy, u_wind.xy) * 0.35 + u_time * 1.9;
        // Two frequencies so the motion never looks like a single sine.
        float sway = sin(phase) * 0.7 + sin(phase * 2.37 + 1.7) * 0.3;
        float gust = 0.65 + 0.35 * fbm2(worldBase.xy * 0.06 + u_time * 0.08, 2);
